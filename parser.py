...
class PartialParse(object):
    ...
    def complete(self):
        ...
        # check if the buffer and stack is empty
        if len(self.sentence) == self.next and len(self.stack) == 1:
            return True
        else:
            return False
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        dependants = [arc[1] for arc in self.arcs if arc[0] == sentence_idx]
        # sort the dependants from left to right
        dependants = sorted(dependants, reverse=False)
        if n is not None:
            dependants = dependants[:n]
    ...
    def get_n_rightmost_deps(self, sentence_idx, n=None):
        ...
        dependants = [arc[1] for arc in self.arcs if arc[0] == sentence_idx]
        # sort the dependants from left to right
        dependants = sorted(dependants, reverse=True)
        if n is not None:
            dependants = dependants[:n]
...
def minibatch_parse(sentences, model, batch_size):
    ...
    """
    # *** BEGIN YOUR CODE ***
    parse_list = [PartialParse(sentence) for sentence in sentences]
    # shallow copy
    unfinished_parse = parse_list[:]
    while len(unfinished_parse) > 0:
        # print(len(unfinished_parse))
        minibatch = unfinished_parse[:batch_size]
        td_pairs = model.predict(minibatch)
        for i in range(len(minibatch)):
            parser = minibatch[i]
            try:
                parser.parse_step(td_pairs[i][0], td_pairs[i][1])
                if parser.complete:
